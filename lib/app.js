...
app.post('/orgaccess', proxy(
    {
      ...
      onProxyRes: (proxyRes, req, res) => {
        ...
        modifyResponse(res, proxyRes, body => {
          if ( body ) {
            // check not null response 
            if ( ! body.data.requestAccessToOrg ) {
              return body;
            }
            // copy the body
            let modified = {
              data: {
                requestAccessToOrg: {
                  accessRequest: {
                    reqID: body.data.requestAccessToOrg.accessRequest.reqId,
                    orgId: body.data.requestAccessToOrg.accessRequest.orgId,
                    orgName: body.data.requestAccessToOrg.accessRequest.orgName,
                    requestorID: body.data.requestAccessToOrg.accessRequest.requestorId,
                    requestStatus: body.data.requestAccessToOrg.accessRequest.requestStatus
                  }
                }
              }
            };
