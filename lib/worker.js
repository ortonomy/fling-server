...
const doNext = async (client, continuous = true) => {
  ...
  try {
    const {
      rows: [job],
    } = await client.query("SELECT * FROM flingapp_jobs.get_job($1, $2);", [
      workerId,
      supportedTaskNames,
    ]);
    if (!job || !job.id) {
    ...
    try {
      if (err) {
        ...
        await client.query("SELECT * FROM flingapp_jobs.fail_job($1, $2, $3);", [
          workerId,
          job.id,
          err.message,
        ]);
        ...
        await client.query("SELECT * FROM flingapp_jobs.complete_job($1, $2);", [
          workerId,
          job.id,
        ]);
      }
